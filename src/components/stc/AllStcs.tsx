...
const AllStcs = () => {
  ...
  return (
    <div className=" flex gap-4 border-[#E4E7EC] border-b-2">
      ...
      <div className={`flex gap-1 p-4 cursor-pointer ${activeDiv === 3 ? 'text-[#FE764B] border-b-2 border-b-[#FE764B] font-bold' : 'text-[#344054] border-inherit'}`}
        onClick={() => setActiveDiv(3)}>
        <p className=" text-sm">Inactive STCs</p>
        {activeDiv === 3&&<div className=" text-[12px] font-medium bg-[#FFE5DD] rounded-[10px] py-1 px-2">429</div>}
      </div>
    </div>
  )
}
