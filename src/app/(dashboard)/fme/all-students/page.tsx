...
export default function Home() {
  ...
  const { unchangedStudentsList, fakeNewStudentId, pageNo } =
  ...
  useEffect(() => {
    ...
    axios
      .get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/student/all`, config) //add page parameter here
      .then((res) => {
  ...
  const uploadCSVData = async (file: File) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/student/create-fme-csv`,
        formData,
        {
          headers: {
  ...
  const handleDownload = async () => {
    ...
    try {
      const response = await axios({
        url: `${process.env.NEXT_PUBLIC_BACKEND_URL}/student/download-csv`,
        method: "GET",
