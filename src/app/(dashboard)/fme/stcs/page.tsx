...
export default function Home() {
	...
	const [stcListDuplicate, setStcListDuplicate] = useState<ISTCData[] | null>(null);
	...
	const handleTabSwitch = (tabIndex: number) => {
		...
		if (tabIndex == 0) {
			if (sortStatus && stcList !== null) {
				const sortedSTCData = sortSTCDataAlphabetically(stcList, sortStatus == "-1");
			...
			if (newStcList) {
				if (sortStatus) {
					const sortedSTCData = sortSTCDataAlphabetically(newStcList, sortStatus == "-1");
					setStcListDuplicate(sortedSTCData);
				} else {
	...
	const CancelQuerySearch = () => {
		...
		if (sortStatus && unchangedStcList !== null) {
			const sortedSTCData = sortSTCDataAlphabetically(unchangedStcList, sortStatus == "-1");
			setStcListDuplicate(sortedSTCData);
			setStcList(unchangedStcList);
		} else {
			setStcListDuplicate(unchangedStcList);
	...
	const handleSearch = (event: FormEvent<HTMLFormElement>) => {
		...
		if (query.trim().length >= 1) {
			...
			const newStcList = unchangedStcList?.filter((ele) => ele.name.toLowerCase().includes(query.toLowerCase()));
	...
	return (
		<>
			...
			{showNewStcFormModal && <NewStcModal cancelModal={() => setShowNewStcFormModal(false)} />}
		</>
	);
}
