...
import {
  CategoryModal,
  NewMdaModal,
} from "@/components/fme/course_list/modals";
...
import {
  CoursesTabSwitches,
  ICourseData,
  sortCourseDataAlphabetically,
} from "@/components/fme/course_list/data";
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        ...
        const maxMdaId = data.reduce(
          (max: number, obj: ICourseCompData) => Math.max(max, obj.Id),
          0
        );
  ...
  const [courseListDuplicate, setCourseListDuplicate] = useState<
    ICourseData[] | null
  >(null);
  const [showCancel, setShowCancel] = useState(false);
  const [courseTabSwitches, setCourseTabSwitches] =
    useState(CoursesTabSwitches);
  ...
  const handleSearch = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (query.trim().length >= 1) {
      const newCourseList = unchangedCoursesList?.filter((ele) =>
      ...
      if (newCourseList && newCourseList.length > 0) {
        ...
        if (sortStatus) {
          const sortedStudentData = sortCourseDataAlphabetically(
            newCourseList,
            sortStatus == "-1"
          );
  ...
  const CancelQuerySearch = () => {
    ...
    if (sortStatus && unchangedCoursesList !== null) {
      const sortedSTCData = sortCourseDataAlphabetically(
        unchangedCoursesList,
        sortStatus == "-1"
      );
    ...
    if (id) {
      ...
      if (id == "1" && courseListDuplicate !== null) {
        const sortedCourseData =
          sortCourseDataAlphabetically(courseListDuplicate);
        setCourseListDuplicate(sortedCourseData);
      } else if (id == "-1" && courseListDuplicate !== null) {
        const sortedCourseData = sortCourseDataAlphabetically(
          courseListDuplicate,
          true
        );
        setCourseListDuplicate(sortedCourseData);
      ...
      <div className="  flex gap-4 border-[#E4E7EC] border-b-2">
        <div
          className={` flex items-center gap-1 p-4 cursor-pointer ${
            activeDiv === 1
              ? "text-[#00932E] border-b-2 border-b-[#00932E] font-bold"
              : "text-[#344054] border-inherit"
          }`}
          onClick={() => setActiveDiv(1)}
        >
          <p className=" text-sm ">Current Course List</p>
          {activeDiv === 1 && (
            <div className=" text-[12px] font-medium bg-[#E7F6EC] rounded-[10px] py-1 px-2">
              {courseListDuplicate?.length}
            </div>
          )}
        </div>
        {/* <div
    ...
    <section className="">
      ...
      <section className="p-4 rounded-lg bg-white mt-4">
        ...
        <div className=" flex flex-wrap gap-2 p-4  ">
          {courseListDuplicate &&
            courseListDuplicate?.map((course) => (
              <CourseCard
      ...
      {showCategory && (
        <CategoryModal cancelModal={() => setShowCategory(false)} />
      )}
    </section>
  );
}
