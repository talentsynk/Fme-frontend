...
>>>>>>> d914e5d35c011101d68e4ffbe286a36282bb0c7b
  ...
  return (
    <section className="bg-white md:px-10 p-4">
      <h2 className=" text-[#191b1c] text-[24px] leading-[32px] font-bold">
        👋 Hello {userData?.FirstName},
      </h2>
      <p className=" text-[#626C70] my-4 font-medium text-sm leading-[20px]">
        Welcome to your dashboard, this is where you get an overview and
      ...
      <section className="border border-[#E4F5EA] border-solid p-4 py-6 rounded-[10px]">
        <h3 className=" text-[24px] mb-2 font-bold leading-[32px] text-black">
          Dashboard
        </h3>
        <div className=" flex flex-col md:flex-row md:justify-between gap-4 items-center md:items-start">
          <div className="md:w-[33%] border border-[#E4F5EA] border-solid w-[95%] flex flex-col gap-4 p-4 bg-white rounded-[12px]">
            <div className=" w-[37px] h-[36px] bg-[#E7F6EC] rounded flex justify-center items-center">
              <Bag />
            </div>
            <h6 className=" text-sm text-black font-medium">
              Total Jobs applied
            </h6>
            <h5 className=" text-[20px] font-medium leading-[30px]">
              {artisanStats?.total_applied_jobs}
            </h5>
          </div>
          <div className="md:w-[33%] border border-[#E4F5EA] border-solid w-[95%] flex flex-col gap-4 p-4 bg-white rounded-[12px]">
            <div className=" w-[37px] h-[36px] bg-[#E7F6EC] rounded flex justify-center items-center">
          ...
          <div className="md:w-[33%] border border-[#E4F5EA] border-solid w-[95%] flex flex-col gap-4 p-4 bg-white rounded-[12px]">
            ...
            <h5 className="text-[20px] font-medium leading-[30px]">
              {artisanStats?.total_jobs_completed}
            </h5>
          </div>
        </div>
      </section>
      <section className=" flex flex-col md:flex-row justify-between py-8">
        <section
          className={`p-4 md:w-[55%] bg-[#E7F6EC] rounded-[10px] ${
            data && data?.length > 0 ? "h-fit" : ""
          }  `}
        >
          <h4 className=" text-[18px] leading-[24px] mb-4 font-bold text-black">
            Jobs applied for
          </h4>
          <section
            className={`${
              data?.length == 0 &&
              " flex w-full justify-center items-center my-auto h-full"
            }`}
          >
            <div
            ...
            >
              {paginatedData && paginatedData.length > 0 ? (
                paginatedData?.map((dum) => (
                  <OragonCard key={dum.Id} {...dum} />
                ))
              ) : (
                <section className=" flex justify-center items-center flex-col gap-8">
                  ...
                  <p className=" md:w-1/2 text-center text-[16px] leading-[24px] text-black font-medium">
                    Sorry but you haven’t applied for any job yet. To apply for
                    a job click on this button{" "}
                  </p>
                  <button
                    onClick={() => router.push("/dashboard/artisan/jobs")}
                    className="w-[200px] h-[48px] rounded-[6px] bg-[#00932E] text-white font-bold"
                  >
                    Apply for Jobs
                  </button>
                </section>
        ...
        <section className="p-4 md:w-[42%]  rounded-[10px]  ">
          <h4 className=" text-[18px] leading-[24px] mb-4 font-bold flex text-black">
            <BigStar />
          ...
          >
            <div
              className={`flex flex-col gap-2 ${
                savedData?.length == 0 && " justify-center items-center"
              }`}
            >
              {SpaginatedData && SpaginatedData.length > 0 ? (
                SpaginatedData.map((dum) => (
                  <SavedOragonCard key={dum.Id} {...dum} />
                ))
              ) : (
                <section className=" flex justify-center items-center flex-col gap-8">
                  <div className=" h-[100px] w-[100px] flex justify-center items-center rounded-[32px] bg-customColorWithOpacity ">
                  ...
                  <p className=" md:w-1/2 text-center text-[16px] leading-[24px] text-black font-medium">
                    Sorry but you haven’t saved any job for later yet. To get
                    started click on this button
                  </p>
                  <button
                    onClick={() => router.push("/dashboard/artisan/jobs")}
                    className="w-[200px] h-[48px] rounded-[6px] bg-[#00932E] text-white font-bold"
